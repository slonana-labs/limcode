...
fn main() {
    ...
    println!("Limcode POD: {:?} ({:.2} GiB/s)",
        limcode_ser_time, 8.0 / limcode_ser_time.as_secs_f64());

    // Wincode
    let start = Instant::now();
    let wincode_bytes = wincode::serialize(&data).unwrap();
    let wincode_ser_time = start.elapsed();
    println!("Wincode:     {:?} ({:.2} GiB/s)",
        wincode_ser_time, 8.0 / wincode_ser_time.as_secs_f64());

    // Bincode
    let start = Instant::now();
    let bincode_bytes = bincode::serialize(&data).unwrap();
    let bincode_ser_time = start.elapsed();
    println!("Bincode:     {:?} ({:.2} GiB/s)",
        bincode_ser_time, 8.0 / bincode_ser_time.as_secs_f64());
    ...
    println!("Limcode POD: {:?} ({:.2} GiB/s)",
        limcode_de_time, 8.0 / limcode_de_time.as_secs_f64());
    ...
    println!("Wincode:     {:?} ({:.2} GiB/s)",
        wincode_de_time, 8.0 / wincode_de_time.as_secs_f64());
    ...
    println!("Bincode:     {:?} ({:.2} GiB/s)",
        bincode_de_time, 8.0 / bincode_de_time.as_secs_f64());
    ...
    println!("Serialize: {:.1}x vs wincode, {:.1}x vs bincode", ser_vs_wincode, ser_vs_bincode);
    println!("Deserialize: {:.1}x vs wincode, {:.1}x vs bincode", de_vs_wincode, de_vs_bincode);
