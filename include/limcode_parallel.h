...
public:
  ...
  std::vector<std::vector<uint8_t>> encode_batch(TxIterator begin, TxIterator end) {
  ...
  std::vector<std::vector<uint8_t>> encode_batch_generic(
 ...
 */
template <typename TxIterator>
inline std::vector<std::vector<uint8_t>> encode_transactions_parallel(
    TxIterator begin, TxIterator end, size_t num_threads = 0) {
 ...
 */
inline void copy_solana_block_parallel(uint8_t* dst, const uint8_t* src, size_t size) {
